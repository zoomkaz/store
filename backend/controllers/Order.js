...
class Order {
  ...
  async create(req, res, next, type) {
    try {
      const { firstName, lastName, email, phone, address, deliweryWay, payOption, confirm, comment = null } = req.body
      // данные для создания заказа
      if (!firstName) throw new Error('Не указано имя покупателя')
      ...
      if (!deliweryWay) throw new Error('Не указан адрес доставки')
      if (!payOption) throw new Error('Не указан адрес доставки')
      ...
      const order = await OrderModel.create({
        firstName, lastName, email, phone, address, deliweryWay, payOption, confirm, comment, items, userId
      })
