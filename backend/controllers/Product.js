...
class Product {
  async getAllFilter(req, res, next) {
    try {
      ...
      const currentLimit = limit && /[0-9]+/.test(limit) && parseInt(limit) ? parseInt(limit) : 10
      const currentPage = page && /[0-9]+/.test(page) && parseInt(page) ? parseInt(page) : 1
      const options = { categoryId, brandId, color, age, weight, price, currentLimit, currentPage, stop }
      const products = await ProductModel.getAllFilter(options)
      res.json(products)
    } catch (e) {
  ...
  async getAll(req, res, next) {
    try {
      ...
      limit = limit && /[0-9]+/.test(limit) && parseInt(limit) ? parseInt(limit) : 10
